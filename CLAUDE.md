...
4. The relevant scope document if working on a specific feature:
   - Foundations: `docs/foundations-scope.md`
   - Narrator: `docs/narrator-scope.md`
   - Scene interrogation: `docs/scene-interrogation-scope.md`
   - Previously On: `docs/previously-on-scope.md`
   - Character management: `docs/character-management-scope.md`
   - World journal: `docs/world-journal-scope.md`

---

## Third-party schema references

The foundry-ironsworn system source is available at `vendor/foundryvtt-ironsworn/`.
...
```
vendor/foundryvtt-ironsworn/src/module/model/actor/
vendor/foundryvtt-ironsworn/src/module/model/item/
```
...
```bash
cd vendor/foundryvtt-ironsworn && git pull origin main && cd ../..
git add vendor/foundryvtt-ironsworn
...
**Key source files to fetch before actor work:**

```bash
# Character schema — all stat, meter, debility, legacy field paths
curl https://raw.githubusercontent.com/ben/foundryvtt-ironsworn/main/src/module/actor/subtypes/character.ts

# Momentum field — MAX, MIN, INITIAL, RESET_MIN constants, burnMomentum()
curl https://raw.githubusercontent.com/ben/foundryvtt-ironsworn/main/src/module/fields/MeterField.ts

# Starship schema — debility.battered, debility.cursed
curl https://raw.githubusercontent.com/ben/foundryvtt-ironsworn/main/src/module/actor/subtypes/starship.ts

# All actor types — character, shared, treasury, foe, site, starship, location
curl https://raw.githubusercontent.com/ben/foundryvtt-ironsworn/main/src/module/actor/config.ts
```
...
- Update `vendor/foundryvtt-ironsworn` without explicit instruction
...
See `vendor/foundryvtt-ironsworn/` for authoritative source.
