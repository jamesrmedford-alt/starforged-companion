 ...
 * src/sectors/sectorArt.js — Background art for sector scenes
 *
 * Generates a 1792×1024 landscape image matching the sector's region character
 * via OpenRouter (FLUX.2 Pro by default). Unlike entity portraits, sector
 * backgrounds:
 ...
 *
 * Every failure mode emits a UI notification (the .catch() in sectorPanel.js
 * is the safety net for unhandled rejections), so failures are visible to the
 * GM without a console capture.
 */
...
// the image model has a clear subject rather than a list of things to avoid.
...
/**
 * Generate a background image for a sector and upload it to Foundry.
 *
...
export async function generateSectorBackground(sector, _campaignState) {
  ...
  console.log(`${MODULE_ID} | sectorArt: starting ` +
  ...
  if (!b64) {
    const msg = "Sector art failed — OpenRouter returned no image bytes. Check the browser console for the raw response shape; the parser may need an update for this model.";
    console.warn(`${MODULE_ID} | sectorArt: ${msg}`);
...
/**
 * Build the image-model prompt for a sector.
 *
 * @param {SectorResult} sector
