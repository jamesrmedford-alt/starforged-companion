...
Injection order (target state — WJ sections 3, 4, 9, 10 are stub functions
returning empty string until World Journal Phase 1 ships):
```
...
Section 3:  Confirmed lore            (WJ — high priority, never dropped)
...
Section 7:  Matched entity cards      (relevance resolver results)
Section 8:  Progress tracks
Section 9:  Faction landscape         (WJ — up to 3 factions, most recent)
Section 10: Recent WJ discoveries     (WJ — current session only)
...
WJ sections (3, 4, 9, 10) are implemented as stub functions returning empty
string in this scope's Phase 2. They consume 0 tokens when empty. No conditional
logic needed in the assembler — the section builder calls the same functions
regardless of whether WJ has shipped. See implementation-ordering.md.
