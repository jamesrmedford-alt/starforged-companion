...
| `companionToolbar.js` | Floating, draggable launcher (frameless `ApplicationV2`) for all Companion panels. Replaces the scene-controls group (v1.7.3); scene-independent so it works with no map open. Exports `openCompanionToolbar()` / `registerCompanionToolbarSettings()`. |
