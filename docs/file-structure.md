...
| `shipMapScene.js` | Battle Stations! ship-map deck-plan Scene (mini-game Phase A). `STATION_LAYOUT` (fixed per-station deck coordinates), `buildStationNoteData` / `buildHullOutlineDrawing` (pure builders), `createShipMapScene` (Scene + 11 station Note pins + schematic hull / art backdrop, mirrors `sectors/sceneBuilder.js`), `generateShipMapForActor` + `maybeCreateShipMapScene` (orchestrator: art → vision → scene → `shipMapSceneId` link; auto-gated), `findShipMapScene`, `isShipMapCommand` (the `!shipmap` matcher), `handleShipStationNoteClick` + `registerShipMapSceneHooks`. Auto-trigger inside `seedStarshipActor`; command IO (`!shipmap`) in `index.js`. |
| `shipMapArt.js` | Deck-plan background art for the ship map. `buildShipMapBackgroundPrompt` (pure, top-down deck-plan prompt seeded from `type`/`firstLook`), `generateShipMapBackground` (OpenRouter image → upload to `worlds/{id}/scenes/`, returns `{path, b64}`). Mirrors `sectors/sectorArt.js`. |
| `shipMapVision.js` | Vision-based station placement. `resolveStationCoordsFromImage` (Claude vision via `api-proxy.js` → normalized per-station coords), `parseJsonObject` + `validateVisionCoords` (pure; all-11/in-range/not-collapsed gate → fixed-layout fallback). Consumed by `shipMapScene.generateShipMapForActor`. |
