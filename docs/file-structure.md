...
| `battleStations.js` | Battle Stations! shipboard-combat framework (rulebook Ch. 3, pp. 184–187) — **not a move**. Pure: `SHIPBOARD_ROLES` (11 crew roles), `buildShipboardCombatGuidance` (narrator block), `shouldInjectShipboardGuidance` (inject predicate), `isBattleStationsCommand` + `renderBattleStationsCardHtml` (the `!stations` play aid). Narrator wiring in `narrator.js`; command IO in `index.js`. Ship-map mini-game planned (`docs/combat/shipboard-combat-minigame.md`). |
