...
| Vow/Combat/Bond Stakes & Rewards | [#241](https://github.com/jamesrmedford-alt/starforged-companion/issues/241) | 📋 PLANNED | Stakes + concrete rewards stated up front at Swear a Vow / Enter the Fray / Forge a Bond; explicit fight→vow link with a difficulty-scaled milestone on a win; fulfilling a vow for a connection deepens it (one-click, scaled by vow rank); combat is offered (Enter the Fray vs. way out) with narrator-suggested difficulty. Two phases. See `decisions.md` → "Vow ↔ combat ↔ bond stakes & rewards" |
