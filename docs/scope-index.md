...
| [Shipboard Combat (Battle Stations!)](combat/shipboard-combat-minigame.md) | 🔄 IN PROGRESS | Battle Stations! (rulebook Ch. 3, pp. 184–187) is the shipboard-combat framework — not a move; the standard combat/suffer/recover moves resolve it, with per-character position and Aid Your Ally handing off control. **Shipped:** narrator-awareness guidance block (11 crew roles, injected when a combat track is open + a command vehicle exists) and the `!stations` player play-aid card (`src/moves/battleStations.js`); **Phase A (v1.7.17)** — a deck-plan ship-map Scene with the 11 stations pinned (`src/moves/shipMapScene.js`), OpenRouter deck-plan art with a schematic hull fallback (`shipMapArt.js`), Claude-vision station placement onto the drawn compartments with a fixed-layout fallback (`shipMapVision.js`), auto-generated on command-vehicle creation (gated, default off) or via `!shipmap`. **Planned (Phases B–D):** "man the stations" token placement, station-aware move suggestions, and integration into the combat lifecycle (see linked doc). Corrects an earlier pass that wrongly struck Battle Stations! as a phantom. | Move resolver, Progress tracks, Narrator, Entity → Actor Migration, Sector Creator Enhanced | decisions.md → "Battle Stations!" |
