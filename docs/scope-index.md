...
| [Entity → Actor Migration](entities/entity-actor-migration-scope.md) | 📋 PLANNED | Migrate ship → starship and planet/settlement/location → location Actors (with `system.subtype` discriminator); hierarchical Actor folders; collapse settlement-data duplication across four storage layers to two; sector-record overview rewritten as UUID document links; one-time GM-triggered `!migrate-entities` with 7-day deferred cleanup | Ironsworn API, Narrator Entity Discovery, Sector Creator | — |
...
- **Entity → Actor Migration (📋 planned)** — see `entities/entity-actor-migration-scope.md`.
